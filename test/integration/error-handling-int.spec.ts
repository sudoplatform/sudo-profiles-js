...
import { DefaultSudoUserClient, internal } from '@sudoplatform/sudo-user'
import FS from 'fs'
...
const clientOptions = {
  graphqlUrl:
    'https://beed3uxqqnc3pmfj3qgah2fiz4.appsync-api.us-east-1.amazonaws.com/graphql',
  region: '',
  tokenProvider: async () => await sudoUserClient.getLatestAuthToken(),
} as const

const graphQLClient = new internal.AmplifyClient({ ...clientOptions })
const mockGraphQLError = (error: any) => {
  jest
    .spyOn(graphQLClient, 'mutate')
    .mockRejectedValue({ originalError: { _response: error } })
}
